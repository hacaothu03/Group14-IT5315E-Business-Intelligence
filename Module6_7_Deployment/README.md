...
`Model_Development/src/predict.py`, exactly as the Module 5 handoff requires.
...
`Model_Development/` and `CleanData/`):
...
out, so `Model_Development/` and `CleanData/` are available.
...
path (`Model_Development/src/predict.py`): `age_at_sale` / `years_since_remodel`
